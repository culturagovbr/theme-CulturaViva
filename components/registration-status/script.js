app.component('registration-status', {
    ...
    methods: {
        isUnderAnalysis(registration) {
            const finalStatuses = [10, 8, 3, 2];
            const hasFinalStatus = finalStatuses.includes(registration.status);

            const consolidated = registration.consolidatedResult;
            const hasConsolidatedResult = consolidated !== null && consolidated !== undefined && consolidated !== '';

            return hasFinalStatus && !hasConsolidatedResult;
        },
        formatNote(note) {
