...
<mc-tab v-if="showTab" label="<?= i::_e('Áreas de atuação') ?>" slug="areas-de-atuacao">
    <mc-container>
        <main>
            <div class="single-data grid-12">
                <h3 class="single-data__title col-12"><?= i::__("Áreas de atuação") ?></h3>

                <template v-for="(values, type) in fields" :key="type">

                    <template v-if="type == 'terms'">
                        <template v-for="(title, taxonomy) in values" :key="taxonomy" >
                            <div v-if="isValid(type, taxonomy)" class="single-data__field col-12">
                                <entity-terms :entity="entity" classes="col-12" :taxonomy="taxonomy" :title="title"></entity-terms>
                            </div>
                        </template>
                    </template>

                    <template v-if="type == 'data'">
                        <template v-if="type == 'data'" v-for="(label, prop) in values" :key="prop" >
                            <div v-if="isValid(type, prop)" class="single-data__field col-12">
