                ...
                <div class="search__tabs--map">
                    ...
                    <search-map type="agent" :pseudo-query="pseudoQuery">
                        <template #filter="{ count }">
                            <search-filter-agent :count="count" :pseudo-query="pseudoQuery" position="map"></search-filter-agent>
                        </template>
                    </search-map>
                </div>
            </mc-tab>

            <mc-tab v-if="global.auth.isLoggedIn"  @click="global.mcTabActive = 'table-collective'"icon="table-view" label="<?php i::esc_attr_e('Tabela (exportar)') ?>" slug="table-collective">
                <agent-table-2 :extra-query="global.auth.is('admin') ? {} : { '@verified': 1 }"></agent-table-2>
            </mc-tab>
        </mc-tabs>
    </template>
</search>